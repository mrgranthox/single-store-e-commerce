...
export const runAdminOpsIntegrationSuite = async () => {
  ...
  try {
    ...
    assert.match(String(publicHomepageResponse.headers["cache-control"] ?? ""), /public/);
    assert.match(String(publicHomepageResponse.headers["cache-control"] ?? ""), /max-age=60/);
    assert.match(String(publicHomepageResponse.headers["x-response-time-ms"] ?? ""), /^\d+(\.\d+)?$/);
