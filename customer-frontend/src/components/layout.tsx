...
───────────────────────────────────────────── */
const supportLinks = [
  { to: "/support", label: "Help Center", icon: "contact_support", exact: true },
  { to: "/account/support", label: "My Tickets", icon: "confirmation_number" },
  { to: "/account/returns", label: "Return Status", icon: "assignment_return" },
  { to: "/account/reviews", label: "My Reviews", icon: "rate_review" },
];

export const SupportLayout = ({ children }: React.PropsWithChildren) => (
  <>
    <header className="bg-slate-50 dark:bg-slate-900 w-full border-none sticky top-0 z-40 safe-area-pt">
      <div className="flex justify-between items-center px-4 sm:px-6 md:px-8 py-3 md:py-4 max-w-7xl mx-auto min-h-14">
        <Link to="/" className="flex items-center gap-2 min-w-0 shrink-0 pr-2">
          <img src={LOGO_SRC} alt={LOGO_ALT} className="h-7 w-auto object-contain rounded-xl dark:opacity-95" width={100} height={32} />
          <span className="text-lg sm:text-xl font-bold text-slate-900 dark:text-slate-50 font-headline tracking-tight truncate">
            {STORE_NAME_SHORT}
          </span>
        </Link>
        <nav className="hidden md:flex items-center gap-8">
          <NavLink to="/support" end className={({ isActive }) => `font-headline tracking-tight font-semibold hover:text-secondary transition-colors ${isActive ? "text-secondary border-b-2 border-secondary pb-1" : "text-slate-600"}`}>Support</NavLink>
          <Link to="/account/orders" className="text-slate-600 font-headline tracking-tight font-semibold hover:text-secondary transition-colors">Orders</Link>
          <Link to="/account/returns" className="text-slate-600 font-headline tracking-tight font-semibold hover:text-secondary transition-colors">Returns</Link>
          <Link to="/account/reviews" className="text-slate-600 font-headline tracking-tight font-semibold hover:text-secondary transition-colors">Reviews</Link>
        </nav>
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          <Link to="/account" className="material-symbols-outlined text-slate-600 dark:text-slate-300 cursor-pointer text-2xl">account_circle</Link>
          <Link to="/cart" className="material-symbols-outlined text-slate-600 dark:text-slate-300 cursor-pointer text-2xl">shopping_bag</Link>
        </div>
      </div>
      <div className="md:hidden border-t border-slate-200/80 dark:border-slate-700 overflow-x-auto no-scrollbar">
        <nav className="flex gap-2 px-4 py-2 min-w-0">
          {supportLinks.map(({ to, label, exact }) => (
            <NavLink
              key={to}
              to={to}
              end={exact}
              className={({ isActive }) =>
                `flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-headline font-semibold whitespace-nowrap ${
                  isActive ? "bg-secondary text-on-secondary" : "bg-slate-200/60 dark:bg-slate-800 text-slate-700 dark:text-slate-200"
                }`
              }
            >
              {label}
    ...
    <div className="flex max-w-7xl mx-auto min-h-screen relative">
      <aside className="hidden md:flex flex-col fixed left-0 top-0 h-full p-4 bg-white w-64 pt-24 z-10">
        <div className="mb-8 px-4">
          <h2 className="text-lg font-black text-slate-900">Support Hub</h2>
          <p className="text-xs text-slate-500">Manage your inquiries</p>
        </div>
        <nav className="flex flex-col gap-1">
          {supportLinks.map(({ to, label, icon, exact }) => (
            <NavLink
              key={to}
              to={to}
              end={exact}
      ...
      <main className="flex-1 md:ml-64 px-4 sm:px-6 md:px-12 py-8 md:py-12 pb-24 md:pb-12 w-full min-w-0 overflow-x-hidden">
        {children}
      </main>
    </div>
    <Footer />
